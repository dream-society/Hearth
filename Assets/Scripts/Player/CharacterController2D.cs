...
public class CharacterController2D : MonoBehaviour
{
    ...
    Dictionary<String, RaycastHit2D> _raycastHitsThisFrameDirectional = new Dictionary<String, RaycastHit2D>() {
        {"left", new RaycastHit2D()},
        {"top", new RaycastHit2D()},
        {"right", new RaycastHit2D()},
        {"bottom", new RaycastHit2D()},
    };
    ...
    void DetectHorizontalCollision() {
		var rayDistance = 0.2f;

		for (var i = 0; i < totalHorizontalRays; i++)
		{
            var ray = new Vector2(_raycastOrigins.bottomRight.x, _raycastOrigins.bottomRight.y + i * _verticalDistanceBetweenRays);

            DrawRay(ray, Vector2.right * rayDistance, Color.red);

            _raycastHit = Physics2D.Raycast(ray, Vector2.right, rayDistance, platformMask);
            if (_raycastHit)
            {
		...
		for (var i = 0; i < totalHorizontalRays; i++)
		{
            var ray = new Vector2(_raycastOrigins.bottomLeft.x, _raycastOrigins.bottomLeft.y + i * _verticalDistanceBetweenRays);

            DrawRay(ray, Vector2.left * rayDistance, Color.red);

            _raycastHit = Physics2D.Raycast(ray, Vector2.left, rayDistance, platformMask);
            if (_raycastHit)
            {
